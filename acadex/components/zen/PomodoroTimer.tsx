...
export default function PomodoroTimer() {
    ...
    return (
        <div
            className={cn(
                "fixed bottom-8 right-8 z-50 transition-all duration-300 ease-in-out bg-black/40 backdrop-blur-xl border border-white/10 shadow-2xl overflow-hidden text-white",
                isMinimized
                    ? "w-16 h-16 rounded-full"
                    : "w-80 rounded-2xl h-auto min-h-[400px]"
            )}
        >
            {isMinimized ? (
                <button
                    onClick={() => setIsMinimized(false)}
                    className="w-full h-full flex items-center justify-center hover:bg-white/10 relative"
                >
                    <div
