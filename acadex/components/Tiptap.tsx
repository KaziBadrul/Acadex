  ...
  return (
    <div className="flex flex-wrap items-center gap-1 p-2 bg-zinc-950 border border-zinc-800 rounded-t-2xl sticky top-0 z-10 select-none">
      ...
      <div className="flex gap-1 pr-2 border-r border-zinc-800">
        ...
        <IconButton onClick={() => editor.chain().focus().setTextAlign("center").run()} isActive={editor.isActive({ textAlign: "center" })} title="Align Center">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="21" x2="3" y1="6" y2="6"/><line x1="19" x2="5" y1="12" y2="12"/><line x1="21" x2="3" y1="18" y2="18"/></svg>
        </IconButton>
        <IconButton onClick={() => editor.chain().focus().setTextAlign("right").run()} isActive={editor.isActive({ textAlign: "right" })} title="Align Right">
      ...
      <div className="flex gap-1 pr-2 border-r border-zinc-800">
        ...
        <IconButton onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} isActive={editor.isActive("heading", { level: 2 })} title="Heading 2">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12h8"/><path d="M4 18V6"/><path d="M12 18V6"/><path d="M21 18h-4c0-4 4-3 4-6 0-1.5-2-2.5-4-1"/></svg>
        </IconButton>
      </div>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto rounded-2xl border border-zinc-800 bg-[#f5f5f5] shadow-xl overflow-hidden">
      ...
      <style jsx global>{`
        ...
        .ProseMirror h2 { font-size: 1.5rem; font-weight: 700; margin-bottom: 1rem; color: white; }
        .ProseMirror p { margin-bottom: 1rem; line-height: 1.75; }
        .ProseMirror ul { list-style: disc; padding-left: 1.5rem; margin-bottom: 1rem; }
        .ProseMirror ol { list-style: decimal; padding-left: 1.5rem; margin-bottom: 1rem; }
        .ProseMirror mark { background-color: #facc15; color: black; padding: 0 2px; border-radius: 2px; }
        .ProseMirror blockquote { border-left: 3px solid #3f3f46; padding-left: 1rem; font-style: italic; color: #a1a1aa; }
      `}</style>
